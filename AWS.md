...
* **R2 endpoint must never leak to the AWS provider:** the R2 remote-state endpoint is the ambient `AWS_ENDPOINT_URL_S3`, present on **all** runs because the s3 backend needs it to reach R2. That var would otherwise also route real `aws_s3_bucket`/`aws_s3_bucket_policy` calls to R2 — which rejects AWS `AKIA` keys with `InvalidArgument: access key has length 20, should be 32`. The safeguard is the `endpoints { s3 = "https://s3.ap-southeast-1.amazonaws.com" }` pin in `provider.tf`, which keeps real buckets in AWS. Do **not** `unset AWS_ENDPOINT_URL_S3` around the mutate targets (that starves the backend) — rely on the pin (see root `README.md` and `docs/incident-2026-08-27-r2-backend-credential-conflict.md`).
