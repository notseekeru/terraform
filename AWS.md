...
* **R2 endpoint must never leak to the AWS provider:** if the R2 remote-state endpoint is ever injected as an ambient `AWS_ENDPOINT_URL_S3`, the AWS provider will route real `aws_s3_bucket`/`aws_s3_bucket_policy` calls to R2 — which rejects AWS `AKIA` keys with `InvalidArgument: access key has length 20, should be 32`. Current fix: the R2 endpoint is delivered **only** to the backend, via the per-module `backend.tfbackend.tpl` rendered at init (see root `README.md`/`Makefile`), so `provider.tf` needs **no** `endpoints.s3` override — real buckets land in AWS by default. If you ever reintroduce the ambient env var, you must re-add an `endpoints.s3` pin to `https://s3.<region>.amazonaws.com` (see `docs/incident-2026-08-27-r2-backend-credential-conflict.md`).
