...
# 5. Apply (full stack: droplets, k8s cluster, database, Helm charts, secrets, ArgoCD app)
make out
make apply
```
...
│   ├── droplet.tf           # Droplets, SSH keys, Ansible inventory
│   ├── kubernetes.tf        # K8s cluster, Helm releases, secrets, ArgoCD, database
...
All targets operate on the `infrastructure/` directory. Plan/apply/destroy automatically load `../secrets.tfvars`.

| Target             | Command                                    | Description                      |
| ------------------ | ------------------------------------------ | -------------------------------- |
| `make init`        | `terraform -chdir=infrastructure init`     | Initialize providers & backend   |
| `make upgradeinit` | `terraform -chdir=infrastructure init -upgrade` | Upgrade initialization      |
| `make plan`        | `terraform -chdir=infrastructure plan ...` | Preview changes                  |
| `make out`         | `... plan -out=tfplan`                     | Save plan to binary file         |
| `make apply`       | `terraform -chdir=infrastructure apply tfplan` | Apply saved plan             |
| `make destroy`     | `terraform -chdir=infrastructure destroy`  | Tear down everything             |
| `make fmt`         | `terraform -chdir=infrastructure fmt`      | Format all `.tf` files           |
| `make validate`    | `terraform -chdir=infrastructure validate` | Validate configuration           |
...
After `apply`, Terraform renders `infrastructure/inventory.tmpl` → `ansible/inventories/droplets.ini`:
...
The `destroy` target also ensures the Kubernetes cluster is fully purged before exiting.
